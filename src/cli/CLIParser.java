...
import cli.command.file.AddFileCommand;
import cli.command.file.PullFileCommand;
import cli.command.file.RemoveFileCommand;
...
import cli.command.organization.AddServentCommand;
import cli.command.organization.RemoveServentCommand;
...
public class CLIParser implements Runnable, Cancellable {
	...
	public CLIParser(SimpleServentListener listener, PingThread pingThread, BackupThread backupThread, List<FifoSendWorker> senderThreads, DistributedMutex mutex) {
		...
		commandList.add(new StopCommand(this, listener, senderThreads, pingThread, backupThread));
		commandList.add(new AddFileCommand(mutex));
		commandList.add(new PullFileCommand());
		commandList.add(new RemoveFileCommand());
		commandList.add(new AddServentCommand());
		commandList.add(new RemoveServentCommand());
